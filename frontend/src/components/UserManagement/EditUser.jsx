...
const EditUser = ({ user, currentUserId, currentUserRole, onClose, onUpdateUser }) => {
  ...
  const getAvailableRoles = () => {
    if (currentUserRole === 'Super Admin') {
      ...
      // Admin cannot assign Admin or Super Admin roles; can only choose Editor/Readonly
      return [
        { label: "Editor", value: "Editor" },
        { label: "Readonly", value: "Readonly" }
      ];
    } else {
      // Editors/Readonly shouldn't access this modal for other users; but for editing self provide limited options
      return [
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      try {
        ...
        // If current user is Admin, they can change roles only to Editor/Readonly and only for non-admin targets
        if (currentUserRole === 'Admin' && !isEditingSelf) {
          if (user.role === 'Admin' || user.role === 'Super Admin') {
            // Admin cannot edit roles of Admins/Super Admins
            // Only phone/address changes should have been allowed, but role change should be blocked
          } else {
            // Allow role change between Editor/Readonly only
            if (formData.role === 'Editor' || formData.role === 'Readonly') {
              updateData.role = formData.role;
            }
          }
        }
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {roleOptions.map((roleOpt) => {
                  ...
                  if (currentUserRole === 'Admin' && (roleOpt.value === 'Admin' || roleOpt.value === 'Super Admin')) {
                    disabled = true;
                  }
                  ...
                  // For Super Admin users being edited by Super Admins (not self), allow role changes
                  if (user.role === 'Super Admin' && isCurrentUserSuperAdmin && !isEditingSelf) {
              ...
              {formData.role === 'Super Admin' && currentUserRole === 'Super Admin' && !isEditingSelf && (
