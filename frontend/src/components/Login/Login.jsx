    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            if (rememberMe) {
                ...
                localStorage.setItem(process.env.REACT_APP_FASTAPITOKEN_KEY,response.fastapi_jwt_token)
            } else {
                sessionStorage.setItem('session', JSON.stringify({
                    token: response.token,
                    expires: Date.now() + 2 * 60 * 60 * 1000 // 2 hours
                }));
                sessionStorage.setItem(process.env.REACT_APP_FASTAPITOKEN_KEY,JSON.stringify(response.fastapi_jwt_token))
            }
