...
const ShowDatabase = ({ connection, onClose }) => {
  ...
  const renderTableData = (data, itemName, totalRows = 0) => {
    if (connection.type === 'MongoDB') {
      // MongoDB collection data (array of documents)
      return (
        <div className="mt-2 bg-gray-50 rounded-lg p-3">
          <h4 className="font-semibold text-sm mb-2">
            Documents ({data.length} of {totalRows.toLocaleString()} total)
          </h4>
          ...
          {totalRows > 5 && (
            <div className="mt-2 text-xs text-gray-500">
              Showing first 5 of {totalRows.toLocaleString()} documents
            </div>
          )}
        </div>
      );
      ...
      return (
        <div className="mt-2 bg-gray-50 rounded-lg p-3 overflow-x-auto">
          <h4 className="font-semibold text-sm mb-2">
            Data ({data.length} of {totalRows.toLocaleString()} total rows)
          </h4>
                          ...
                          {expandedItems[collection] && (
                            <div className="p-3 border-t border-gray-200">
                              {loadingTable[collection] ? (
                                ...
                                renderTableData(tableData[collection].data, collection, tableData[collection].totalRows)
                              ) : (
                          ...
                          {expandedItems[table] && (
                            <div className="p-3 border-t border-gray-200">
                              {loadingTable[table] ? (
                                ...
                                renderTableData(tableData[table].data, table, tableData[table].totalRows)
                              ) : (
                                <div className="text-sm text-gray-500">No data available</div>
                              )}
                            </div>
