...
exports.login = async (req, res, next) => {
  try {
    ...
    const fastapi_jwt_token = GenerateToken(
      {
        email: user.email,
        role: user.role,
        name: user.name
    ...
    if (fastapi_jwt_token.success) {
      ...
      res.json({
        success: true,
        token,
        fastapi_jwt_token:fastapi_jwt_token.token,
        user: {
          id: user.id,
          name: user.name,
          email: user.email,
      ...
      const err = new Error(fastapi_jwt_token.message);
      next(err);
    }
